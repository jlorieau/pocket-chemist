...
class Processor(metaclass=ProcessorMeta):
    ...
    def __init__(self, name=None, *args, **kwargs):
        ...
        if len(missing_params) > 0:
            raise ValueError(
                f"The {self.__class__.__name__} processor is "
                f"missing the following required kwargs "
                f"parameters: {missing_params}"
            )

        # Collect the specified parameters
        available_params = set(self.required_params) | set(self.optional_params)
    ...
    def print(
        self, level: int = 0, space_level: int = 0, item_number: t.Optional[str] = ""
    ) -> None:
...
class GroupProcessor(Processor):
    ...
    def add(self, processor: Processor):
        ...
        if isinstance(processor, Processor):
            ...
            raise TypeError(
                f"The {self.__class__.__name__} class can only "
    ...
    def print(self, level=0, space_level=space_level):
        ...
        for count, processor in enumerate(self.processors, 1):
            processor.print(level=level + 1, space_level=space_level, item_number=count)
